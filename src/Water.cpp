...
std::shared_ptr<Water> Water::create()
{
    ...
    // so doesnt matter, let it be like that for now
...
Water::Water(AK::Badge<Water>, std::shared_ptr<Material> const& material) : Model(material)
{
    ...
    normal_map1 = ResourceManager::get_instance().load_texture("./res/textures/water/water_normal1.jpg", TextureType::Diffuse);
    normal_map2 = ResourceManager::get_instance().load_texture("./res/textures/water/water_normal2.jpg", TextureType::Diffuse);
    Water::prepare();
}
...
void Water::draw() const
{
    ...
    renderer->get_device_context()->PSSetShaderResources(18, 1, &normal_map1->shader_resource_view);
    renderer->get_device_context()->PSSetShaderResources(19, 1, &normal_map2->shader_resource_view);
    set_constant_buffer();
