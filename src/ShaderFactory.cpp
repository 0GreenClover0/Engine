...
std::shared_ptr<Shader> ShaderFactory::create(std::string const& vertex_path, std::string const& tessellation_control_path,
                                              std::string const& tessellation_evaluation_path, std::string const& fragment_path)
{
    ...
    case Renderer::RendererApi::OpenGL:
    {
        auto shader = std::make_shared<ShaderGL>(AK::Badge<ShaderFactory> {}, vertex_path, tessellation_control_path,
                                                 tessellation_evaluation_path, fragment_path);
    ...
    case Renderer::RendererApi::DirectX11:
    {
        auto shader = std::make_shared<ShaderDX11>(AK::Badge<ShaderFactory> {}, vertex_path, tessellation_control_path,
                                                   tessellation_evaluation_path, fragment_path);
