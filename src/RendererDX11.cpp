...
void RendererDX11::create_rasterizer_state()
{
    ...
    hr = g_pd3dDevice->CreateRasterizerState(&wfdesc, &g_rasterizer_state);
    if (FAILED(hr))
    {
...
void RendererDX11::create_depth_stencil()
{
    ...
    hr = g_pd3dDevice->CreateTexture2D(&depth_stencil_desc, NULL, &m_depth_stencil_buffer);
    if (FAILED(hr))
    {
        std::cout << "Failed to create depth stencil texture." << "\n";
    }
    hr = g_pd3dDevice->CreateDepthStencilView(m_depth_stencil_buffer, NULL, &m_depth_stencil_view);
    if (FAILED(hr))
    {
