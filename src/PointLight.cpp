...
void PointLight::set_up_shadow_mapping()
{
    ...
    shadow_texture_desc.Width = static_cast<float>(renderer->SHADOW_MAP_SIZE);
    shadow_texture_desc.Height = static_cast<float>(renderer->SHADOW_MAP_SIZE);
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        assert(SUCCEEDED(hr));
        m_shadow_depth_stencil_views.push_back(shadow_depth_stencil_view);
    }

}

void PointLight::set_render_target_for_shadows(int face) const
{   
    auto renderer = RendererDX11::get_instance_dx11();
    renderer->get_device_context()->OMSetRenderTargets(1, &renderer->g_emptyRenderTargetView, m_shadow_depth_stencil_views[face]);
    renderer->get_device_context()->ClearDepthStencilView(m_shadow_depth_stencil_views[face], D3D11_CLEAR_DEPTH, 1.0f, 0);
}

glm::mat4 PointLight::get_projection_view_matrix(int face)
{
    if (entity->transform->get_model_matrix() != m_last_model_matrix)
    {
        update_pv_matrices();
    }
    return projection_view_matrices[face];
}
...
void PointLight::update_pv_matrices()
{
    ...
    float aspect = (float)renderer->SHADOW_MAP_SIZE / (float)renderer->SHADOW_MAP_SIZE;
    glm::mat4 shadow_proj = glm::perspective(glm::radians(90.0f), aspect, m_near_plane, m_far_plane);
    ...
    glm::vec3 light_pos = transform->get_position();
    projection_view_matrices.clear();
    projection_view_matrices.push_back(shadow_proj *
