...
{
	std::shared_ptr<TextureLoaderDX11> textureLoader = std::make_shared<TextureLoaderDX11>();
	set_instance(textureLoader);
	return textureLoader;
}

void TextureLoaderDX11::clean_up()
{
...
{
	HRESULT hr;
	// i am reconsidering my life choices after writing this line
	auto device = static_cast<RendererDX11*>(Renderer::get_instance().get())->get_device();
	...
	unsigned char* image_data = stbi_load(path.c_str(),
		&image_width,
		&image_height,
	...
	int image_pitch = image_width * image_channels; 
	...
	hr = device->CreateTexture2D(&image_texture_desc,
	...
	hr = device->CreateShaderResourceView(g_image_texture,
		nullptr,
		&g_image_shader_resource_view
	);
	assert(SUCCEEDED(hr));
	...
	hr = device->CreateSamplerState(&image_sampler_desc,
		&g_image_sampler_state);
...
GLint TextureLoaderDX11::convert_wrap_mode(TextureWrapMode const wrap_mode)
{
	return GLint();
}

GLint TextureLoaderDX11::convert_filtering_mode(TextureFiltering const texture_filtering)
{
	return GLint();
}
