...
float4 ray_cast(float3 dir, inout float3 hitCoord)
{
    dir *= ray_step;
    ...
    int steps;
    float4 projected_coord;
    float dDepth; // wtf is this shit supposed to mean
    [loop]
    for(int i = 0; i < max_steps; ++i)
    {
        hitCoord += dir;

        projected_coord = mul(projection, float4(hitCoord, 1.0f));
        // perspective divide
        projected_coord.xy /= projected_coord.w;
        // transform to NDC
        projected_coord.xy = projected_coord.xy * 0.5f + 0.5f;
        ...
        if((dir.z - dDepth) < 1.0f)
        {
            ...
            {
                float4 result;
                result = float4(binary_search(dir, hitCoord, dDepth), 1.0f);
                return result;
            }
        }
        steps++;
